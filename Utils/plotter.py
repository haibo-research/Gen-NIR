...
def plot_multiple_directories(log_dirs: List[str], csv_name: str = 'training.csv', col: str = 'loss',
                              fname: str = 'summary.png', window_size: int = 5, labels: List[str] = None,
                              title: str = '', init_epoch: int = 0, last_epoch: int = -1) -> NoReturn:
    """
...
def hausdorff(verts, faces, verts_target, faces_target):
    tree1 = cKDTree(verts)
    tree2 = cKDTree(verts_target)
    dist1, _ = tree1.query(verts_target)
    dist2, _ = tree2.query(verts)
...
def mesh_volume(verts, faces):
    """使用矢量化方法计算由顶点和面定义的封闭网格的体积，并确保三角形顶点顺序正确"""
    v1 = verts[faces[:, 0]]
    v2 = verts[faces[:, 1]]
    v3 = verts[faces[:, 2]]
    ...
    # 确保三角形顶点顺序正确（右手规则）
    # 计算每个三角形的中心点与法向量的点积
    dot_product = np.einsum('ij,ij->i', (v1 + v2 + v3) / 3, normals)

    # 如果点积为负，说明顶点顺序需要调整
    need_swap = dot_product < 0
    v2[need_swap], v3[need_swap] = v3[need_swap], v2[need_swap]

    # 重新计算调整后的法向量
    normals = np.cross(v2 - v1, v3 - v1)

    # 计算有向体积
    volumes = np.einsum('ij,ij->i', v1, normals) / 6.0
    total_volume = np.abs(np.sum(volumes))
    return total_volume

def volume_diff(verts1, faces1, verts2, faces2):

    """计算两个网格的体积差"""
    volume1 = mesh_volume(verts1, faces1)
    volume2 = mesh_volume(verts2, faces2)
...
def plot_error(log_dir: str, csv_name: str = 'optimization.csv',
    ...
    # # 设置纵坐标名称
    # ax1.set_ylabel('y')
    ax3.plot(df['epoch'], df['hausdorff distance'], c='b')
    plt.savefig(os.path.join(log_dir, 'hausdorff_distance.png'))
    plt.close()
    ...
    # # 设置纵坐标名称
    # ax1.set_ylabel('y')
    ax4.plot(df['epoch'], df['volume difference'], c='b')
    plt.savefig(os.path.join(log_dir, 'volume_difference.png'))
    plt.close()