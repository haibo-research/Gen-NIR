...
def decode_sdf(decoder, latent_vector, queries):
    ...
    input = torch.cat([latent_repeat, queries.to(device)], dim=1)
    sdf = decoder(input)
    return sdf
...
def indicator_plane(
    ...
    decoder.eval()
    with torch.no_grad():
        input = decoder(torch.cat([batch_vecs, xyz], dim=1).to(torch.float64).to(device)).detach().cpu()
...
def create_mesh_with_edge(decoder, latent, N=32, l=0.05):
    ...
    decoder.eval()
    with torch.no_grad():
        input = decoder(torch.cat([batch_vecs.float(), xyz.float()], dim=1).to(torch.float64))
    numpy_3d_sdf_tensor = input.squeeze().detach().cpu().numpy().reshape(N, N, N)
    verts, faces, normals, values = skimage.measure.marching_cubes(
        numpy_3d_sdf_tensor, level=l, spacing=[N] * 3
    )
