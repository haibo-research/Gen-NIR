import sys
sys.path.append('/home/hbliu/Desktop/DeepSDF_EIT/')
import argparse
import signal
...
parser.add_argument("--log_dir", default=root + '/logs/DeepSDF_EIT/pancreas_' + time.strftime('%m_%d_%H%M%S'))
parser.add_argument("--LogFrequency", default=10)
# data para
parser.add_argument("--DataSource", default=root + "/data")
# parser.add_argument("--TrainSplit", default=root + "/data/splits/sv2_spheres_train.json")
parser.add_argument("--TrainSplit", default=root + "/data/splits/sv2_pancreas_train.json")
parser.add_argument("--SamplesPerScene", default=16384)
parser.add_argument("--ScenesPerBatch", default=4)
...
def main_function():
    ...
    with open(join(experiment_directory, 'args.json'), 'w', newline='\n') as f:
        json.dump(dict(vars(specs)), f, indent=1)
    ...
    arch = __import__("networks." + specs.NetworkArch, fromlist=["DeepSDF"])
    decoder = arch.DeepSDF(latent_size, **specs.NetworkSpecs).to(device)
    # decoder = NET(latent_size).to(device)
    print(
    ...
    for epoch in training_loop:
        ...
        if epoch % log_frequency == 0:
            plot_all_iterations(experiment_directory)
            plot_train_test([experiment_directory])
            torch.save(
            ...
            if epoch > 0:
                ...
                for ith in range(0, 100, 10):
                    sdf_data, indices = sdf_dataset.__getitem__(ith)
                    dirs = experiment_directory + '/reconstruction/' + str(ith) + '/'
                    if not os.path.exists(dirs):
                        os.makedirs(dirs)
                    latent = lat_vecs(torch.Tensor([indices]).int()).to(device)
                    mesh_filename = dirs + str(epoch) + '.html'
                    with torch.no_grad():
                        verts, faces, normals = create_mesh_with_edge(decoder, latent.squeeze(), N=32)
                        write_verts_faces_to_file(verts, faces, mesh_filename)
