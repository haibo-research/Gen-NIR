...
from deep_sdf import plot_all_iterations, plot_train_test, create_mesh_with_edge, write_verts_faces_to_file
...
def main_function(experiment_directory, data_source, continue_from, batch_split):
    ...
    if continue_from is None:
        ...
        with open(join(experiment_directory, 'args.json'), 'w', newline='\n') as f:
            json.dump(dict(specs), f, indent=1)
    ...
    lr_schedules = deep_sdf.lr_schedule.get_learning_rate_schedules(specs)
    ...
    if continue_from is not None:
        ...
        epoch_log = open(join(experiment_directory, 'train_test.csv'), 'a')
        training_log = open(join(experiment_directory, 'train_iterations.csv'), 'a')
        with open(join(experiment_directory, 'args.json'), 'a', newline='\n') as f:
            json.dump(dict(specs), f, indent=1)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        if epoch % log_frequency == 0:
            ...
            try:
                for ith in range(0, 60, 10):
                    sdf_data, indices = sdf_dataset.__getitem__(ith)
                    dirs = experiment_directory + '/reconstruction/' + str(ith) + '/'
                    if not os.path.exists(dirs):
                        os.makedirs(dirs)
                    latent = lat_vecs(torch.Tensor([indices]).int()).squeeze().cuda()
                    mesh_filename = dirs + str(epoch) + '.html'
                    with torch.no_grad():
                        verts, faces, normals = create_mesh_with_edge(decoder, latent.detach(), N=32,
                                                                                     l=0.005)
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument(
        ...
        "-d",
        dest="data_source",
        default='/home/hbliu/Desktop/DeepSDF_EIT/data',
