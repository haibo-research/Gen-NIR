...
parser.add_argument("--sdf_log_dir", default=root + '/pretrained/DeepSDF_plane')
...
parser.add_argument("--grid_r", default=1.5)
...
def Shape_Derivative(verts, faces, args, grid_obs, obs_dirichlet_vertices, obs_neumann_element):
    ...
    for kk in range(args.num_current):
        ...
        # solving
        (adjoint_neumann_init, adjoint_neumann_obs), info, it_count = \
            bempp.api.linalg.gmres(blocked, [rhs_fun1, rhs_fun2], tol=args.tol, return_iteration_count=True)
        ...
        # solving
        (state_neumann_init, state_dirichlet_obs), info, it_count = \
            bempp.api.linalg.gmres(blocked, [rhs_fun1, rhs_fun2], tol=args.tol, return_iteration_count=True)
...
def optimize(latent_init, args, decoder, grid_obs, grid_target, obs_dirichlet_vertices, obs_neumann_element, obs_neumann_vertices, indicator_target,
    ...
    for epoch in training_loop:
        ...
        print('%d,%.5f,%.5f,%.5f,%.5f' % (epoch, loss_epoch, indicator_error, hausdorff_distance, volume_difference), file=epoch_log, flush=True)
